...
当前版本：`0.3.0`。本版本完成了领域逻辑、脚本渲染、GUI 校验、测试和 Windows 打包流程的统一整理。

## 主要能力

- 序列模式：按时间点、目标电压、DOD 容量进度组织充放电与 EIS。
- 脉冲模式：支持弛豫、脉冲、脉冲后 EIS 和尾段电压放电。
- 统一容量基准：容量冲突会被阻断，不会继续生成不可信脚本。
- 风险控制：错误阻止复制，高风险 Warning 需要确认后才能复制。
- 纯命令输出：最终脚本不包含公式、注释或调试文本，可直接粘贴到 CHI Macro Command。
- 可验证发布：提供 Windows EXE、自动化测试、覆盖率检查和 SHA256 校验文件。
...
- 包名：[CHI-OPEIS-0.3.0-windows-x64.zip](dist/CHI-OPEIS-0.3.0-windows-x64.zip)
- 主程序：`CHI-OPEIS.exe`
- 注意：不要只单独复制 exe，必须保留解压后的完整目录，尤其是同级 `_internal/` 目录。

校验发布包时，请使用同目录的 `SHA256SUMS.txt`，不要手工复制固定哈希。

## 三种 EIS 取点语义

| 模式 | 过程控制 | EIS 初始状态 | 适用场景 |
| --- | --- | --- | --- |
| 目标电压 | CP 到达目标电压 | 使用目标电压 `ei` | 比较明确电压状态下的阻抗 |
| 电压弛豫 | CP 触发电压状态变化 | 使用 `eio` 读取当前状态 | 观察弛豫后的实际状态 |
| DOD 准原位 | 按统一容量基准运行到目标进度 | 使用 `eio` 读取当前状态 | 按容量进度比较阻抗 |

CP 的实际结束时间取决于电池响应，因此电压模式的总历时或 SoC 预测可能显示为未知或不完整；软件会明确标记这一点，不伪造精确时间。
...
8. 没有阻断错误后，先确认高风险 Warning，再复制 **极简脚本** 到 CHI Macro Command。
